...
插件自身的版本号从 0.1.7 线起跟随宿主：`<dsh 版本>-v<本插件序号>`，例如 `0.1.7-rc.1-v0.1`；发布必须显式使用 `next` tag，因为该版本在 semver 上低于旧线的 `0.8.x`。规则细节与递增方式见[开发与 RC 发布](docs/development.md#版本号规则017-线起)。
